...
export async function generateMetadata({ searchParams }) {
  ...
  return {
    title: `Video Popular${pageLabel} - ${siteName}`,
    description: `Daftar video bokep terpopuler berdasarkan jumlah penonton terbanyak di ${siteName}.${pageLabel}`,
    alternates: {
      canonical: `${siteUrl}/popular${page > 1 ? `?page=${page}` : ''}`,
    },
    openGraph: {
      title: `Video Popular${pageLabel} - ${siteName}`,
      description: `Daftar video bokep terpopuler di ${siteName}.`,
...
export default async function PopularPage({ searchParams }) {
  ...
  const [videos, totalVideos] = await Promise.all([
    Video.find().sort({ views: -1 }).skip(skip).limit(limit).lean(),
    ...
    <>
      <div className="flex items-center justify-between mb-6 border-b border-gray-800 pb-4">
        <div className="flex items-center gap-3">
          <div className="w-1.5 h-8 bg-yellow-500 rounded-full shadow-[0_0_10px_rgba(234,179,8,0.5)]"></div>
