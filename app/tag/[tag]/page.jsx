...
// Cache tag pages for 30 minutes
...
export async function generateMetadata({ params, searchParams }) {
  ...
  const seoDescription = `Kumpulan video ${displayTag} dengan berbagai jenis adegan. Koleksi ${displayTag} terlengkap. Update terbaru setiap hari. Nonton video ${displayTag} gratis tanpa iklan di ${siteName}.`;

  return {
    title: `${displayTag}${pageLabel} - ${siteName}`,
...
export default async function TagPage({ params, searchParams }) {
  ...
  const query = { tags: { $regex: tag.replace(/-/g, ' '), $options: 'i' } };
    ...
    <>
      <div className="flex items-center justify-between mb-8 border-b border-gray-800 pb-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-black text-white tracking-tight uppercase">
            Tag: <span className="text-pink-500">#{displayTag}</span>
          </h2>
        </div>
        <div className="hidden md:block text-xs text-gray-500 font-medium bg-gray-900 px-3 py-1 rounded-full border border-gray-800">
          {totalVideos.toLocaleString()} Video Ditemukan
        </div>
      </div>

      <div className="mb-8 bg-gray-900/50 border border-gray-800 rounded-xl p-4 md:p-6">
        <p className="text-gray-300 leading-relaxed text-sm md:text-base">
          Kumpulan video #{displayTag} dengan berbagai jenis adegan.
          Koleksi #{displayTag} terlengkap.
          Update terbaru setiap hari. Nonton video #{displayTag} gratis tanpa iklan di {siteName}.
        </p>
      </div>
      ...
      {videos.length > 0 ? (
            ...
            <div className="flex justify-center items-center gap-2 mt-8 mb-20">
              {page > 1 && (
                <Link 
                  ...
                  className="w-10 h-10 flex items-center justify-center bg-gray-800 text-white rounded-lg hover:bg-pink-600 transition shadow-lg border border-gray-700"
                >
                  &larr;
                </Link>
              )}
              ...
              <div className="px-6 py-2 bg-gray-900 border border-gray-800 rounded-lg text-sm font-medium shadow-inner">
                <span className="text-pink-500 font-bold">{page}</span>
        ...
        <div className="min-h-[50vh] flex flex-col items-center justify-center text-center bg-gray-900/50 border border-gray-800 rounded-2xl p-10">
          ...
          <h3 className="text-xl font-bold text-white mb-2">Belum ada video</h3>
          <p className="text-gray-500 mb-6">
            Maaf, belum ada video dengan tag <span className="text-pink-500">#{displayTag}</span>.
          </p>
